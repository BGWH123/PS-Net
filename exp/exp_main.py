...
class Exp_Main(Exp_Basic):
    ...
    def _build_model(self):
        ...
        print(f"Total Parameters: {params/1e3:.2f}K")
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        ...
                        if not torch.isfinite(loss):
                            print("=" * 60)
                            print(f"[FATAL] Loss exploded (NaN / Inf)")
...
# keep only the last variable
        ...
        # denorm_preds = np.array([test_data.inverse_transform(p) for p in preds])
        # denorm_trues = np.array([test_data.inverse_transform(t) for t in trues])

        # with open(self.args.recoder_name, "a") as ft:
        #     ft.write(f"Setting: {setting}\n")
        #     ft.write(f"Predicted_denorm: {denorm_preds[0, :, -1].tolist()}\n")
        #     ft.write(f"True_denorm: {denorm_trues[0, :, -1].tolist()}\n")
        #     ft.write(f"Predicted: {preds[0, :, -1].tolist()}\n")
        #     ft.write(f"True: {trues[0, :, -1].tolist()}\n")
        #     ft.write("\n")
# keep only the last variable
        ...
        print(
            'mae:{}, rmse:{}, mse:{}, rse:{}'.format(
                mae, rmse, mse, rse
            )
        )

        f = open(f"{self.args.result_name}", 'a')
        f.write(setting + "  \n")
        f.write(
            'mae:{}, rmse:{}, mse:{}, rse:{}'.format(
                mae, rmse, mse, rse
